...
parser.add_argument("--gpu_id", type=int, default=-1, help="Which gpu id to use")
...
def write_output(cityscapes, hist_perframe):
    ...
    with open(args.output_dir + '/evaluation_results.txt', 'w') as f:
        ...
        for i, cl in enumerate(cityscapes.classes):
            while len(cl) < 15:
                cl = cl + ' '
...
def main():
    ...
    for i, idx in enumerate(label_frames):
        if i % 10 == 0:
            ...
            write_output(CS, hist_perframe)
            print('Wrote output to ' + args.output_dir + '/evaluation_results.txt')
        city = idx.split('_')[0]
